...
* 属于操作系统参数
* 表示一个进程可以打开的文件描述符的数量，高并发需要保持大量连接，所以需要调大点，linux参数



#### 4）TCP_NODELAY

* 属于 SocketChannal 参数 默认false（开启）  建议为 true
* 该算法为了提高效率，把小数据包会攒几个一起发，但导致没有那么实时



#### 5）SO_SNDBUF & SO_RCVBUF

- 滑动窗口的大小参数，系统会自动调整，现在不用设置
...
* ```
  系统环境变量 
  io.netty.allocator.type = "unpooled" (非池化)   "pooled"（池化）
  io.netty.noPreferDirect = "false" (系统内存)   "true" （堆内存）
  ```
